...
Real getEnergy(std::vector<ITensor> mps) {
  ...
  for (int i = start; i > 1; i-=2) {
      ...
      mps[i - 1] = U*D;
      //mps[i - 1] /= norm(mps[i - 1]);
      U = ITensor(commonIndex(mps[i-1], mps[i - 2]));
      svd(mps[i - 1], U, D, V, {"Cutoff", cutoff, "Maxm", maxm});
      mps[i - 2] = mps[i - 2]*U*D;
      //mps[i - 2] /= norm(mps[i - 2]);
      mps[i - 1] = V;
  }
  normalizeCheck(phi, N);
  normalizeCheck(mps, N);
  ITensor psi = ITensor(1.0);
  ITensor psi2 = ITensor(1.0);
  for (int i = 0; i < N; i++) {
      //Print(mps[i]);
      psi = psi*mps[i];
      psi2 = psi2*phi[i];
  }
  return (dag(psi)*psi2).real();
}
...
printfln("\nGround state energy by TEBD= %.10f", getEnergy(mps));
